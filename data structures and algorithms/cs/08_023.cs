...
public class IntervalCoverageOptimization {
    public static int MinTeamMembers(int[][] intervals, int K) {
        ...
        while (!AllCovered(covered)) {
            ...
            for (int i = 0; i < intervals.Length; i++) {
                if (!covered[i] && intervals[i][0] >= shiftStart && intervals[i][1] <= shiftEnd) {
                    covered[i] = true;
                }
            }
        }
    ...
    public static void Main(string[] args) {
        int[][] intervals = {
            new int[] { 1, 3 }, new int[] { 2, 5 }, new int[] { 6, 8 }, new int[] { 8, 10 }, new int[] { 11, 12 }
        };
        int K = 5;
        Console.WriteLine("Minimum team members needed: " + MinTeamMembers(intervals, K)); // Output: 2
    }
}