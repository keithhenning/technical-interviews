...
public class Graph
{
    ...
    public void AddEdge(string fromNode, string toNode, int weight)
    {
        if (weight < 0)
        {
            throw new ArgumentException("Negative weights not allowed in Dijkstra's");
        }
    ...
    public KeyValuePair<Dictionary<string, int>, Dictionary<string, List<string>>> Dijkstra(string start)
    {
        ...
        {
            throw new ArgumentException($"Start node '{start}' not in graph");
        }
    ...
    public static void Main(string[] args)
    {
        ...
        foreach (var entry in paths)
        {
            ...
            if (node != startNode)
            {
                Console.WriteLine($"  To {node}: {string.Join(" -> ", path)} (distance: {distances[node]} miles)");
            }
        }
    }
}
