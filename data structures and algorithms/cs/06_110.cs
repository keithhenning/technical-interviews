...
public class FloydWarshall
{
    ...
    public static void Main(string[] args)
    {
        ...
        if (path.Count > 0)
        {
            Console.WriteLine("Path from 0 to 3: " + string.Join(" -> ", path));
        }
    }
    ...
    public static FloydWarshallResult FloydWarshallAlgorithm(int[][] graph)
    {
        int n = graph.Length;
        int[][] dist = new int[n][];
        int?[][] next = new int?[n][];
        ...
        return new FloydWarshallResult { Distances = dist, NextVertex = next };
    }

    public static List<int> GetPath(int start, int end, int?[][] next)
    {
