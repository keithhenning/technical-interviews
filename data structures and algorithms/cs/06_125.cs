...
public class AStar {
    ...
    */
   public static List<int[]> AStarAlgorithm(int[,] grid, int[] start, int[] goal) {
      ...
      while (openSet.Count > 0) {
         ...
         string currentKey = ArrayToString(currentPos);
         
         // Check if reached goal
         if (ArraysEqual(currentPos, goal)) {
            // Reconstruct path
            List<int[]> path = new List<int[]>();
            path.Add(currentPos);
            while (cameFrom.ContainsKey(currentKey)) {
               currentPos = cameFrom[currentKey];
               currentKey = ArrayToString(currentPos);
         ...
         foreach (int[] neighbor in GetNeighbors(currentPos, grid)) {
            string neighborKey = ArrayToString(neighbor);
   ...
   public static string VisualizePath(int[,] grid, List<int[]> path, int[] start, int[] goal) {
      System.Text.StringBuilder visual = new System.Text.StringBuilder();
   ...
   public static void Main(string[] args) {
      ...
      // Find path using A*
      List<int[]> path = AStarAlgorithm(grid, start, goal);
