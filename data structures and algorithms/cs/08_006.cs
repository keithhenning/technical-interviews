...
{
   public string OptimalDocumentSegment(
      string document,
      string tokens)
   {
      if (string.IsNullOrEmpty(document) ||
         string.IsNullOrEmpty(tokens))
      {
         return "";
      }

      // Count frequencies of tokens
      Dictionary<char, int> tokenCount =
      ...
      Dictionary<char, int> windowCount =
         new Dictionary<char, int>();
      ...
      while (right < document.Length)
      {
         ...
         // Check if character satisfies token requirement
         if (tokenCount.ContainsKey(c) &&
         ...
         while (left <= right && formed == requiredTokens)
         {
            ...
            // Check if removing breaks token requirement
            if (tokenCount.ContainsKey(c) &&
               windowCount[c] < tokenCount[c])
            {
               formed--;
      ...
      return minLen == int.MaxValue ? "" :
         document.Substring(resultStart, minLen);
   }
}