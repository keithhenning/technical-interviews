...
def get_resource():
    ...
    user_tier = get_user_tier(user_id)  # This would be a separate function
    if rate_limiter.is_allowed(user_id, user_tier):
        # Process the request
        return jsonify({"data": "Your requested resource"})
    else:
        # Rate limited
        retry_after = 60  # Simplified - in production, calculate actual wait time
        return jsonify({
...
def get_user_tier(user_id):
    ...
    return "premium" if user_id.startswith("premium_") else "free"
